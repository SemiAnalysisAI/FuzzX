...
| [m003-shl3-add-chain](known-miscompiles/m003-shl3-add-chain/NOTES.md) | FAIL | PASS | FAIL | `-O0` scalarizes a divergent `shl3/add` chain through `v_readfirstlane_b32`. |
| [m004-vector-identity-xor](known-miscompiles/m004-vector-identity-xor/NOTES.md) | FAIL | PASS | FAIL | `-O0` loses a lane-0 vector identity before `xor`. |
| [m005-shl1-add-chain](known-miscompiles/m005-shl1-add-chain/NOTES.md) | FAIL | PASS | FAIL | `-O0` scalarizes a divergent `shl1/add` chain through the same class of bug as m003. |
| [m006-i8-xor-clear](known-miscompiles/m006-i8-xor-clear/NOTES.md) | FAIL | PASS | FAIL | `-O0` lowers another adjacent `i8` narrow byte-clear xor through the wrong `v_bitop3_b32` result. |
| [m007-vector-shl-identity-xor](known-miscompiles/m007-vector-shl-identity-xor/NOTES.md) | FAIL | PASS | FAIL | `-O0` loses a vector shift-by-zero lane-0 identity before `xor`. |
| [m008-i8-separated-clear](known-miscompiles/m008-i8-separated-clear/NOTES.md) | FAIL | PASS | FAIL | `-O0` miscompiles an `i8` identity byte-clear xor when prior narrow ops are separated by no-op adds. |
| [m009-i16-clear-xor](known-miscompiles/m009-i16-clear-xor/NOTES.md) | FAIL | PASS | FAIL | `-O0` miscompiles an `i16` identity low-16 clear xor through the wrong `v_bitop3_b32` result. |
| [m010-i16-sext-clear-xor](known-miscompiles/m010-i16-sext-clear-xor/NOTES.md) | FAIL | PASS | FAIL | `-O0` miscompiles an `i16` sign-extended identity clear xor through the wrong `v_bitop3_b32` result. |
| [m011-i8-sext-clear-xor](known-miscompiles/m011-i8-sext-clear-xor/NOTES.md) | FAIL | PASS | FAIL | `-O0` miscompiles an `i8` sign-extended masked clear xor through the wrong `v_bitop3_b32` result. |
| [m012-add-shl-ladder](known-miscompiles/m012-add-shl-ladder/NOTES.md) | FAIL | PASS | FAIL | `-O0` scalarizes a divergent `add/shl` ladder through `v_readfirstlane_b32`. |
| [m013-private-memory-fshl](known-miscompiles/m013-private-memory-fshl/NOTES.md) | FAIL | FAIL | FAIL | `-O0` lowers fixed private-memory allocas through a dynamic scratch stack sequence that can return intermittent wrong values. |
| [m014-shl-add-ctpop](known-miscompiles/m014-shl-add-ctpop/NOTES.md) | FAIL | PASS | FAIL | `-O0` scalarizes a four-step `shl/add` chain feeding `ctpop` through lane 0. |
| [m015-scalar-fshl-zero](known-miscompiles/m015-scalar-fshl-zero/NOTES.md) | FAIL | PASS | FAIL | `-O0` lowers scalar `fshl.i32(x, y, 0)` through a 64-bit shift-by-`-1` sequence that returns zero. |
| [m016-scalar-fshl-one](known-miscompiles/m016-scalar-fshl-one/NOTES.md) | FAIL | PASS | FAIL | `-O0` lowers scalar `fshl.i32(x, y, 1)` through a 64-bit shift-by-`-1` sequence that returns only bit 31. |
