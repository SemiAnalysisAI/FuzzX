...
| [m129-performfnegcombine-fcanonicalize-flips-nan-sign](known-miscompiles/m129-performfnegcombine-fcanonicalize-flips-nan-sign/NOTES.md) | ❌ | ❌ | ❌ | `performFNegCombine` FCANONICALIZE arm (`AMDGPUISelLowering.cpp:5402-5427`) folds `fneg(fcanonicalize x) -> fcanonicalize(fneg x)` (and the double-fneg collapse) with no `nnan` gate. HW `fcanonicalize` (`v_max_f32 x, x`) returns +qNaN for any NaN input regardless of sign. Original IR: `fneg(fcanon(-qNaN)) = -qNaN`. Folded: `fcanon(fneg -qNaN) = fcanon(+qNaN) = +qNaN`. O0 stores `0xFFC00000`, O2 stores `0x7FC00000`. Sibling of m107/m110/m111/m120/m127/m128. |
