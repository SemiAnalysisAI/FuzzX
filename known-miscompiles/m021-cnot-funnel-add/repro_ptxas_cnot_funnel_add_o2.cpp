...
//   g++ -std=c++17 -O2 repro_ptxas_cnot_funnel_add_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_cnot_funnel_add_o2
//
...
//
// Run:
//   ./repro_ptxas_cnot_funnel_add_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_cnot_funnel_add_o2 sm_103
//
...
//
//     r0 = n                         = 0x00000020
//     r0 = cnot.b32(r0)              = 0x00000000
//     r0 = shf.r.wrap.b32(r0,16,19)  = 0x00020000
...
//   -O0 output: 0x00acd528, which matches the scalar trace.
//   -O2 output: 0x00a8d528, which is wrong. This is exactly 0x40000 lower,
//   as if the optimized shf.r.wrap+add fold subtracted the 0x20000 high
//   funnel contribution instead of adding it.
//
// SASS root-cause summary:
//   -O0 emits SHF.R.W.U32 followed by IADD3. -O2 folds the expression into a
//   LEA.HI sequence (`LEA.HI R5, -R0, R5, 0x10, 0xd`) that produces
//   0x00a8d528 for n = 32 instead of the PTX-required 0x00acd528.
...
.entry fuzz_kernel(.param .u64 out_ptr, .param .u32 n)
{
    ...
    cnot.b32       %r0, %r0;
    shf.r.wrap.b32 %r0, %r0, 16, 19;
    add.u32        %r0, %r0, 0xaad528;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_cnot_funnel_repro.XXXXXX");
...
static uint32_t shf_r_wrap(uint32_t a, uint32_t b, uint32_t count) {
    count &= 31;
    if (count == 0) {
        return a;
    }
    return (a >> count) | (b << (32 - count));
}

static uint32_t expected_value(uint32_t n) {
    uint32_t r0 = (n == 0) ? 1 : 0; // cnot.b32
    r0 = shf_r_wrap(r0, 16, 19);
    return r0 + 0x00aad528u;
}
