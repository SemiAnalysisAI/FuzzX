...
//   g++ -std=c++17 -O2 repro_ptxas_slct_s32_immediate_o1.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_slct_s32_immediate_o1
//
...
//
// Run:
//   ./repro_ptxas_slct_s32_immediate_o1 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_slct_s32_immediate_o1 sm_103
//
// Correct scalar behavior:
//   PTX `slct.dtype.s32 d, a, b, c` stores `a` in `d` when signed `c >= 0`,
//   otherwise it stores `b`. This kernel executes:
//
//     slct.s32.s32 %r0, 1, 12, 2142016040
//
//   The selector 2142016040 is 0x7fac9228, a positive signed 32-bit value.
//   The correct selected value is therefore the first source operand, 1.
//
// Observed bug with CUDA 13.0 ptxas V13.0.88 on sm_103 and with CUDA 13.2
// Update 1 ptxas V13.2.78 on sm_103:
//   -O0 stores 0x00000001.
//   -O1, -O2, and -O3 store 0x0000000c.
//
// SASS root-cause summary:
//   At -O0, ptxas emits a predicate test for `0 <= 0x7fac9228` followed by
//   `SEL`, which chooses 1. At -O1 and above, ptxas folds the instruction to
//   the constant 12, exactly as if it had reversed the constant-folding rule
//   for the positive immediate selector of `slct.s32.s32`.
...
constexpr uint32_t EXPECTED = 0x00000001u;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_slct_s32_repro.XXXXXX");
