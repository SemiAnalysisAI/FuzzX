...
//   g++ -std=c++17 -O2 repro_ptxas_neg_loop_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_neg_loop_o2
//
...
//
// Run:
//   ./repro_ptxas_neg_loop_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_neg_loop_o2 sm_103
//
// Correct scalar behavior:
//   The first counted loop executes once and only decrements its counter:
//
//     r4 = 1
//     r4 = r4 - 1 = 0
//
//   After that loop, the kernel sets:
//
//     r3 = 0x7fffffff
//     r6 = 1
//
//   The second counted loop also executes once and applies `neg.s32`:
//
//     r6 = r6 - 1 = 0
//     r3 = neg.s32(0x7fffffff) = 0x80000001
//
//   PTX integer negation is two's-complement modulo 2^32 for this input; no
//   overflow trap or undefined behavior is involved. The correct stored value
//   is therefore 0x80000001.
//
// Observed bug with CUDA 13.0 ptxas V13.0.88 on sm_103 and with CUDA 13.2
// Update 1 ptxas V13.2.78 on sm_103:
//   -O0 stores 0x80000001.
//   -O2 stores 0x7fffffff, as if the second loop's `neg.s32` did not run.
//
// SASS root-cause summary:
//   At -O0, ptxas keeps both counted loops. The second loop contains:
//
//     IADD3 R4, PT, PT, RZ, -R4, RZ ;
//
//   which is the expected negation of 0x7fffffff to 0x80000001.
//
//   At -O2, ptxas removes both one-trip loops but stores the pre-negation
//   constant directly:
//
//     MOV   R7, 0x7fffffff ;
//     STG.E ..., R7 ;
//
//   The optimized cubin has dropped the side effect of the second counted
//   loop. If the preceding counted loop is removed from the source, ptxas
//   handles the one-trip `neg.s32` loop correctly, so this is a loop
//   simplification interaction with sequential counted loops.
...
constexpr uint32_t EXPECTED = 0x80000001u;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_neg_loop_repro.XXXXXX");
