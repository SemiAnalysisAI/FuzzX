// Standalone ptxas -O2/-O3 miscompile reproducer.
//
// This file embeds a reduced PTX kernel, assembles it with ptxas (-O0, -O1,
// -O2, and -O3), launches one CUDA thread through the CUDA Driver API, and
// compares the output against a scalar PTX trace.
//
// Build, typical x86 CUDA install:
//   g++ -std=c++17 -O2 repro_ptxas_abs_loop_bmsk_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_abs_loop_bmsk_o2
//
...
//
// Run:
//   ./repro_ptxas_abs_loop_bmsk_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_abs_loop_bmsk_o2 sm_103
//
// Correct scalar behavior:
//   The kernel launches one thread, so tid.x = 0. The loop runs three times.
//   In every iteration r10 is 0, so:
//
//     r5 = r10 - 1 = 0xffffffff
//     r2 = abs.s32(r5) = 1
//
...
//     r3 >> 8 = 0x00003fe0
//     r6 = r2 + r3 = 0x00000001 + 0x00003fe0 = 0x00003fe1
//
//   The correct output is therefore 0x00003fe1.
//
// Observed bug with CUDA 13.0 ptxas V13.0.88 on sm_103 and with CUDA 13.2
// Update 1 ptxas V13.2.78 on sm_103:
//   -O0 and -O1 store 0x00003fe1.
//   -O2 and -O3 store 0x00003fdf.
//
// SASS root-cause summary:
//   At -O0, ptxas keeps the loop and computes the live-out value with IABS,
//   then adds +1 to bmsk(13, 9) >> 8. At -O2, ptxas deletes the loop and emits
//   BMSK followed by LEA.HI, storing (bmsk(13, 9) >> 8) - 1. That is exactly
//   what would happen if the loop folder used the pre-abs value -1 instead of
//   the post-abs value +1 for the post-loop add.
...
constexpr uint32_t EXPECTED = 0x00003fe1u;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_abs_loop_bmsk_repro.XXXXXX");
