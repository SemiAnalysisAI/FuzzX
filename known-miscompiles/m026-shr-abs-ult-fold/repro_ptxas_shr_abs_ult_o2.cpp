...
// in_ptr parameter is kept so the entry uses the same three-argument ABI as the
// original fuzzer testcase.
//
// Build, typical x86 CUDA install:
//   g++ -std=c++17 -O2 repro_ptxas_shr_abs_ult_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_shr_abs_ult_o2
//
...
//
// Run:
//   ./repro_ptxas_shr_abs_ult_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_shr_abs_ult_o2 sm_103
//
...
//   r0 = p0 ? 1 : 2      = 1
//
// ptxas -O0 stores 1. With affected ptxas versions, ptxas -O2 folds away the
// predicate and stores 2. This reproduced on 2026-05-15 with CUDA Toolkit 13.0
// ptxas V13.0.88 and CUDA Toolkit 13.2 Update 1 ptxas V13.2.78, which was the
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_shr_abs_ult_repro.XXXXXX");
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "arch:  " << arch << "\n";
        std::cout << "expected scalar PTX output: " << EXPECTED << "\n";
        std::cout << "-O0 output: " << o0 << "\n";
        std::cout << "-O2 output: " << o2 << "\n";
