...
//   g++ -std=c++17 -O2 repro_ptxas_addc_mul_carry_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_addc_mul_carry_o2
//
...
//
// Run:
//   ./repro_ptxas_addc_mul_carry_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_addc_mul_carry_o2 sm_103
//
// Correct PTX behavior for n = 32:
//   r0 = n * 512              = 0x00004000
//   r1 = r0 - 72              = 0x00003fb8
//   r1 = 8388608 * r1         = 0xdc000000  (low 32 bits)
//   add.cc.u32 r2, r1, 31     has carry-out 0
//   addc.u32 r3, r0, -9       = 0x00004000 + 0xfffffff7 + 0 = 0x00003ff7
//
// ptxas -O0 stores 0x00003ff7. With affected ptxas versions, ptxas -O2 feeds
// an incorrect carry into addc and stores 0x00003ff8. This reproduced on
...
constexpr uint32_t EXPECTED = 0x00003ff7u;
...
.entry fuzz_kernel(.param .u64 in_ptr, .param .u64 out_ptr, .param .u32 n)
{
    ...
    ld.param.u32 %r0, [n];
    mul.lo.u32 %r0, %r0, 512;
    add.u32 %r1, %r0, 4294967224;
    mul.lo.u32 %r1, 8388608, %r1;
    add.cc.u32 %r2, %r1, 31;
    addc.u32 %r3, %r0, 4294967287;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_addc_mul_repro.XXXXXX");
