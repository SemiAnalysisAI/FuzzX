...
//   g++ -std=c++17 -O2 repro_ptxas_not_clz_predicate_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_not_clz_predicate_o2
//
...
//
// Run:
//   ./repro_ptxas_not_clz_predicate_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_not_clz_predicate_o2 sm_103
//
// Correct PTX behavior for the one-thread launch:
//   r0 = tid.x                    = 0
//   r1 = mul.hi.u32(2, r0)        = 0
//   r2 = not.b32 r1               = 0xffffffff
//   p0 = (r1 != 0)                = false, so the branch is not taken
//   r3 = clz.b32 r2               = clz(0xffffffff) = 0
//
// ptxas -O0 stores 0x00000000. With affected ptxas versions, ptxas -O2 stores
// 0x00000020, as if it folded the false path to clz(0) and dropped the not.
...
constexpr uint32_t EXPECTED = 0x00000000u;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_not_clz_repro.XXXXXX");
