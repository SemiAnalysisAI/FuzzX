...
//   g++ -std=c++17 -O2 repro_ptxas_bfind_loop_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_bfind_loop_o2
//
...
//
// Run:
//   ./repro_ptxas_bfind_loop_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_bfind_loop_o2 sm_103
//
// Correct scalar behavior:
//   The first counted loop executes once and only decrements its counter.
//   The second counted loop executes four times. Each iteration sets:
//
...
//
//   The correct stored value is therefore 0x80000000.
//
// Observed bug with CUDA 13.0 ptxas V13.0.88 on sm_103 and with CUDA 13.2
// Update 1 ptxas V13.2.78 on sm_103:
//   -O0 stores 0x80000000.
//   -O2 stores 0x7ffffffe, as if it folded the final subtract as an add.
//
// SASS root-cause summary:
//   At -O0, ptxas keeps the loops and lowers bfind.u32 0 to `FLO.U32 R2, RZ`,
//   then emits `IADD3 R4, ..., R2, -R4, ...` for the final subtraction.
//   At -O2, ptxas deletes the loops and stores constant 0x7ffffffe directly.
//   Replacing `bfind.u32 0` with `mov.u32 0xffffffff` makes -O2 correct, so
//   this is a loop simplification/folding interaction involving bfind.
...
constexpr uint32_t EXPECTED = 0x80000000u;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_bfind_loop_repro.XXXXXX");
