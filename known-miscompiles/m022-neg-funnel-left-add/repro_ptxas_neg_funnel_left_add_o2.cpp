...
//   g++ -std=c++17 -O2 repro_ptxas_neg_funnel_left_add_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_neg_funnel_left_add_o2
//
...
//
// Run:
//   ./repro_ptxas_neg_funnel_left_add_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_neg_funnel_left_add_o2 sm_103
//
...
//     r0 = n                         = 0x00000020
//     r0 = 0 - r0                    = 0xffffffe0
//     r0 = shf.l.wrap.b32(r0,0,28)   = 0x0ffffffe
//     r0 = r0 + 1                    = 0x0fffffff
//
...
//   -O0 output: 0x0fffffff, which matches the scalar trace.
//   -O2 output: 0xffffffff, which is wrong.
//
// SASS root-cause summary:
//   -O0 emits SHF.L.W.U32.HI followed by IADD3. -O2 folds the expression into
//   `LEA.HI R5, -R5, 0x1, RZ, 0x1c`, which sign-extends this case instead of
//   producing the logical funnel-shift extraction required by PTX.
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_neg_funnel_left_repro.XXXXXX");
...
static uint32_t shf_l_wrap(uint32_t a, uint32_t b, uint32_t count) {
    count &= 31;
    if (count == 0) {
        return b;
    }
    return (a >> (32 - count)) | (b << count);
}

static uint32_t expected_value(uint32_t n) {
    uint32_t r0 = 0u - n;
    r0 = shf_l_wrap(r0, 0, 28);
    return r0 + 1;
}
