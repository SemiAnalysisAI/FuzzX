...
//   g++ -std=c++17 -O2 repro_ptxas_addc_shift_carry_o1.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_addc_shift_carry_o1
//
...
//
// Run:
//   ./repro_ptxas_addc_shift_carry_o1 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_addc_shift_carry_o1 sm_103
//
// Correct scalar behavior:
//   The host passes in_n = 32. The kernel computes:
//
//     r9  = 32 << 1              = 0x00000040
//     r9  = r9 << 31             = 0x00000000  (low 32 bits of 2^37)
//     r11 = add.cc.u32 r9, 536   = 0x00000218 with carry-out 0
//     r9  = addc.u32 64, 0       = 0x00000040 + carry-in 0
//
//   The correct output is therefore 0x00000040.
//
// Observed bug with CUDA 13.0 ptxas V13.0.88 on sm_103 and with CUDA 13.2
// Update 1 ptxas V13.2.78 on sm_103:
//   -O0 stores 0x00000040.
//   -O1, -O2, and -O3 store 0x00000041.
//
// SASS root-cause summary:
//   At -O0, ptxas preserves the carry-producing add and the carry-consuming
//   addc, so the carry-in to addc is the real carry-out of 0 + 536, which is 0.
//   At -O1 and above, ptxas folds the shifted value to zero but folds the addc
//   as if the carry-in were 1, producing 65 instead of 64.
...
constexpr uint32_t EXPECTED = 0x00000040u;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_addc_shift_repro.XXXXXX");
