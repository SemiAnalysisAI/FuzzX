...
| `seed.bin`       | Bytes that drove `fuzzx_execgen::generate_from_bytes`.          |
...
```bash
./target/release/fuzzx-diff-verify known-miscompiles/<name>
```
...
`crates/fuzzx-diff/src/bin/` where they exist.

## Environment

* Generator: `fuzzx-execgen` defaults (see `crates/fuzzx-execgen/src/lib.rs`).
...
Independent verification: `cargo run -p fuzzx-diff --release --bin fuzzx-diff-oracle-050f`
