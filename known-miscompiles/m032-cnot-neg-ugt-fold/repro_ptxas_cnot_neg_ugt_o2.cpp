...
//   g++ -std=c++17 -O2 repro_ptxas_cnot_neg_ugt_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_cnot_neg_ugt_o2
//
...
//
// Run:
//   ./repro_ptxas_cnot_neg_ugt_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_cnot_neg_ugt_o2 sm_103
//
// Correct PTX behavior for the one-thread launch:
//   r0 = tid.x                    = 0
//   r1 = cnot.b32 r0              = 1
//   r1 = neg.s32 r1               = 0xffffffff
//   p0 = setp.gt.u32 0, r1        = false, because unsigned 0 > 0xffffffff
//   r2 = selp.b32 1, 2, p0        = 2
//
// ptxas -O0 stores 0x00000002. With affected ptxas versions, ptxas -O2 stores
// 0x00000001, as if it folded the unsigned predicate to tid.x == 0. This
// reproduced on 2026-05-15 with CUDA Toolkit 13.0 ptxas V13.0.88 and CUDA
// Toolkit 13.2 Update 1 ptxas V13.2.78, which was the latest NVIDIA CUDA
...
constexpr uint32_t EXPECTED = 0x00000002u;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_cnot_neg_repro.XXXXXX");
