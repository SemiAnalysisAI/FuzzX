...
//   g++ -std=c++17 -O2 repro_ptxas_max_chain_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_max_chain_o2
//
...
//     r1 = max.s32(-96, 0) = 0
//
//   Therefore the only correct output is 0x00000000.
//
// Observed bug with CUDA 13.0 ptxas V13.0.88 on sm_103 and with CUDA 13.2
// Update 1 ptxas V13.2.78 on sm_103:
//   -O0 output: 0x00000000, which matches the scalar trace.
//   -O2 output: 0x00000020, which is wrong. This is as if ptxas introduced
//   the original positive value n as a signed-max candidate, even though the
//   PTX only maxes post-subtract values 0, -n, -2n, and -3n.
//
// SASS root-cause summary:
//   -O0 keeps the recurrence as IADD3 + VIMNMX pairs.
//   -O2 combines the chain into VIADDMNMX/VIMNMX3:
//
//     VIADDMNMX R0, R4, -R4, RZ, !PT ;  // max(n - n, 0) = 0
//     VIMNMX3   R0, R0, R4, R5, !PT ;   // max(0, n, -2n) = n  <-- wrong
//
//   The source chain never computes max(..., n), so the optimized SASS has an
//   illegal extra candidate.
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_max_chain_repro.XXXXXX");
...
static uint32_t max_s32(uint32_t a, uint32_t b) {
