...
// out[20][3] against the scalar PTX trace. It does not read an input buffer;
// the dummy in_ptr and in_n parameters are kept to match the fuzzer ABI.
//
// Build, typical x86 CUDA install:
//   g++ -std=c++17 -O2 repro_ptxas_not_xor_branch_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_not_xor_branch_o2
//
...
//
// Run:
//   ./repro_ptxas_not_xor_branch_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_not_xor_branch_o2 sm_103
//
// Correct PTX behavior for lane tid.x == 20:
//   p0  = setp.ne.u32 tid.x, 20       = false
//   r13 = not.b32 tid.x               = ~20 = 0xffffffeb
//   r4  = xor.b32 r13, 0x9033a9b3     = 0x6fcc5658
//   store r4 to out[20][3]
//
// All other lanes branch to exit without writing. The output buffer is zeroed
// before launch, so the only relevant word is out[20][3].
//
// ptxas -O0 stores 0x6fcc5658. With affected ptxas versions, ptxas -O2 stores
// 0x9033a9a7, exactly 20 ^ 0x9033a9b3, as if the optimizer dropped the
// not.b32. This reproduced on 2026-05-15 with CUDA Toolkit 13.0 ptxas
// V13.0.88 and CUDA Toolkit 13.2 Update 1 ptxas V13.2.78, which was the latest
// NVIDIA CUDA Toolkit listed that day.
//
...
constexpr uint32_t EXPECTED = 0x6fcc5658u;
constexpr uint32_t WRONG = 0x9033a9a7u;
constexpr size_t OUTPUT_BYTES = 32 * 16;
constexpr size_t TARGET_OFFSET = 20 * 16 + 12;
...
)
{
    ...
    mov.u32      %r20, %tid.x;
    setp.ne.u32  %p0, %r20, 20;
    @%p0 bra     exit;

    not.b32      %r13, %r20;
    xor.b32      %r4, %r13, 0x9033a9b3;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_not_xor_repro.XXXXXX");
...
static uint32_t launch(const std::vector<char>& cubin) {
    ...
    check(cuLaunchKernel(func, 1, 1, 1, 32, 1, 1, 0, nullptr, params, nullptr),
          "cuLaunchKernel");
