// Standalone ptxas -O1/-O2/-O3 miscompile reproducer.
//
// This file embeds a reduced PTX kernel, assembles it with ptxas (-O0, -O1,
// -O2, and -O3), launches one 32-thread CUDA block through the CUDA Driver API,
// and compares each thread's output against a scalar PTX trace.
//
// Build, typical x86 CUDA install:
//   g++ -std=c++17 -O2 repro_ptxas_structured_loop_uniform_counter_o1.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_structured_loop_uniform_counter_o1
//
// Build, CUDA SBSA install like this machine:
//   g++ -std=c++17 -O2 repro_ptxas_structured_loop_uniform_counter_o1.cpp \
//     -I/usr/local/cuda/targets/sbsa-linux/include \
//     -L/usr/local/cuda/targets/sbsa-linux/lib/stubs -lcuda \
//     -o repro_ptxas_structured_loop_uniform_counter_o1
//
// Run:
//   ./repro_ptxas_structured_loop_uniform_counter_o1 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_structured_loop_uniform_counter_o1 sm_103
//
// Correct scalar behavior:
//   The kernel is launched with exactly 32 threads, so %tid.x is 0..31.
//   The first counted loop executes once because r39 starts at 1. The branch
//   on p42 is never taken for launched threads because p42 = (%tid.x == 32).
//   The next branch splits tid 31 from tids 0..30 because p50 = (%tid.x == 31);
//   both sides only execute zero-trip counted loops for the launched threads.
//
//   After that split, all threads reach the final nested counted loop. r44 and
//   r45 both start at 1, so every thread must execute the final loop body
//   exactly once:
//
//     r1 = 1 | tid
//
//   The correct slot-0 output for tid t is therefore (t | 1).
//
// Observed bug with CUDA 13.0 ptxas V13.0.88 on sm_103 and CUDA 13.2 Update 1
// ptxas V13.2.78 on sm_103:
//   -O0 stores the correct per-thread values.
//   -O1, -O2, and -O3 store stale 1 for tids 2..30.
//
// SASS root-cause summary:
//   At -O0, ptxas keeps the final counted-loop predicates in per-lane
//   registers and emits the final LOP3/OR in the loop body.
//
//   At -O1 and above, ptxas promotes the final loop counters r44/r45 into
//   uniform registers UR4/UR5. The preceding control flow is still divergent:
//   tid 31 reaches the final loop as a separate subgroup. That subgroup
//   decrements the uniform loop counters to zero. When tids 0..30 arrive, they
//   see the already-zero uniform counters and skip the final OR, storing the
//   stale value 1 instead of tid|1.

#include <cuda.h>

#include <cerrno>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <fstream>
#include <iostream>
#include <sstream>
#include <stdexcept>
#include <string>
#include <sys/wait.h>
#include <unistd.h>
#include <vector>

constexpr int N_THREADS = 32;
constexpr int OUTPUT_WORDS_PER_THREAD = 4;
constexpr int OUTPUT_WORDS = N_THREADS * OUTPUT_WORDS_PER_THREAD;
constexpr int OUTPUT_BYTES = OUTPUT_WORDS * static_cast<int>(sizeof(uint32_t));

static const char* kPtx = R"PTX(
.version 8.8
.target sm_103
.address_size 64

.visible .entry fuzz_kernel(
    .param .u64 in_ptr,
    .param .u64 out_ptr
)
{
        .reg .pred  %p<66>;
        .reg .b32   %r<46>;
        .reg .b64   %rd<6>;

    ld.param.u64    %rd1, [out_ptr];
    mov.u32         %r24, %tid.x;
    mov.u32         %r39, 1;
    mov.u32         %r40, 0;
    mov.u32         %r41, 0;
    mov.u32         %r42, 0;
    mov.u32         %r43, 1;
    mov.u32         %r44, 1;
    mov.u32         %r45, 1;


structured_loop_19_header:
    setp.eq.u32   %p41, %r39, 0;
    @%p41 bra   structured_loop_19_done;
    sub.u32         %r39, %r39, 1;
    setp.eq.u32   %p42, %r24, 32;
    @%p42 bra   structured_if_20_then;
    bra             structured_if_20_else;
structured_if_20_then:
structured_loop_21_header:
    setp.eq.u32   %p45, %r40, 0;
    @%p45 bra   structured_loop_21_done;
    sub.u32         %r40, %r40, 1;
    bra             structured_loop_21_header;
structured_loop_21_done:
    bra             structured_if_20_done;
structured_if_20_else:
    bra             structured_if_20_done;
structured_if_20_done:
    setp.eq.u32   %p50, %r24, 31;
    @%p50 bra   structured_if_22_then;
    bra             structured_if_22_else;
structured_if_22_then:
structured_loop_23_header:
    setp.eq.u32   %p51, %r41, 0;
    @%p51 bra   structured_loop_23_done;
    sub.u32         %r41, %r41, 1;
    bra             structured_loop_23_header;
structured_loop_23_done:
    bra             structured_if_22_done;
structured_if_22_else:
structured_loop_25_header:
    setp.eq.u32   %p58, %r42, 0;
    @%p58 bra   structured_loop_25_done;
    sub.u32         %r42, %r42, 1;
    setp.eq.u32   %p59, %r24, 32;
    @%p59 bra   structured_if_26_then;
    bra             structured_if_26_else;
structured_if_26_then:
    bra             structured_if_26_done;
structured_if_26_else:
structured_loop_27_header:
    setp.eq.u32   %p63, %r43, 0;
    @%p63 bra   structured_loop_27_done;
    sub.u32         %r43, %r43, 1;
    bra             structured_loop_27_header;
structured_loop_27_done:
    bra             structured_if_26_done;
structured_if_26_done:
    bra             structured_loop_25_header;
structured_loop_25_done:
    bra             structured_if_22_done;
structured_if_22_done:
    bra             structured_loop_19_header;
structured_loop_19_done:
structured_loop_28_header:
    setp.eq.u32   %p64, %r44, 0;
    @%p64 bra   structured_loop_28_done;
    sub.u32         %r44, %r44, 1;
structured_loop_29_header:
    setp.eq.u32   %p65, %r45, 0;
    @%p65 bra   structured_loop_29_done;
    sub.u32         %r45, %r45, 1;
    or.b32        %r1, 1, %r24;
    bra             structured_loop_29_header;
structured_loop_29_done:
    bra             structured_loop_28_header;
structured_loop_28_done:
    bra             exit;

exit:
    cvta.to.global.u64 %rd4, %rd1;
    mul.wide.u32    %rd5, %r24, 16;
    add.s64         %rd4, %rd4, %rd5;
    st.global.u32   [%rd4 + 0], %r1;
    ret;
}
)PTX";

static void check(CUresult r, const char* op) {
    if (r == CUDA_SUCCESS) {
        return;
    }
    const char* msg = nullptr;
    cuGetErrorString(r, &msg);
    std::ostringstream os;
    os << op << " failed: " << (msg ? msg : "unknown CUDA error");
    throw std::runtime_error(os.str());
}

struct TempDir {
    std::string path;
    explicit TempDir(const char* pattern) {
        std::vector<char> buf(pattern, pattern + std::strlen(pattern) + 1);
        char* p = mkdtemp(buf.data());
        if (!p) {
            throw std::runtime_error(std::string("mkdtemp failed: ") + std::strerror(errno));
        }
        path = p;
    }
    ~TempDir() {
        unlink((path + "/in.ptx").c_str());
        unlink((path + "/out.cubin").c_str());
        rmdir(path.c_str());
    }
};

static void write_text(const std::string& path, const char* text) {
    std::ofstream f(path);
    if (!f) {
        throw std::runtime_error("failed to open " + path);
    }
    f << text;
}

static std::vector<char> read_binary(const std::string& path) {
    std::ifstream f(path, std::ios::binary);
    if (!f) {
        throw std::runtime_error("failed to open " + path);
    }
    return std::vector<char>(std::istreambuf_iterator<char>(f),
                             std::istreambuf_iterator<char>());
}

static std::vector<char> compile_ptx(const std::string& ptxas,
                                     const std::string& arch,
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_structured_loop_repro.XXXXXX");
    const std::string ptx_path = dir.path + "/in.ptx";
    const std::string cubin_path = dir.path + "/out.cubin";
    write_text(ptx_path, kPtx);

    pid_t pid = fork();
    if (pid < 0) {
        throw std::runtime_error(std::string("fork failed: ") + std::strerror(errno));
    }
    if (pid == 0) {
        std::string arch_arg = "-arch=" + arch;
        execlp(ptxas.c_str(), ptxas.c_str(), arch_arg.c_str(), opt,
               "-o", cubin_path.c_str(), ptx_path.c_str(), static_cast<char*>(nullptr));
        std::perror("execlp ptxas");
        _exit(127);
    }

    int status = 0;
    if (waitpid(pid, &status, 0) < 0) {
        throw std::runtime_error(std::string("waitpid failed: ") + std::strerror(errno));
    }
    if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
        std::ostringstream os;
        os << "ptxas " << opt << " failed with status " << status;
        throw std::runtime_error(os.str());
    }
    return read_binary(cubin_path);
}

static void create_context(CUcontext* ctx, CUdevice dev) {
#if CUDA_VERSION >= 13000
    check(cuCtxCreate(ctx, nullptr, 0, dev), "cuCtxCreate");
#else
    check(cuCtxCreate(ctx, 0, dev), "cuCtxCreate");
#endif
}

static std::vector<uint32_t> run_kernel(const std::vector<char>& cubin) {
    CUmodule module = nullptr;
    CUfunction fn = nullptr;
    CUdeviceptr d_dummy = 0;
    CUdeviceptr d_out = 0;
    std::vector<uint32_t> out(OUTPUT_WORDS, 0xa5a5a5a5u);

    check(cuModuleLoadData(&module, cubin.data()), "cuModuleLoadData");
    check(cuModuleGetFunction(&fn, module, "fuzz_kernel"), "cuModuleGetFunction");
    check(cuMemAlloc(&d_dummy, 4), "cuMemAlloc dummy");
    check(cuMemAlloc(&d_out, OUTPUT_BYTES), "cuMemAlloc output");
    check(cuMemsetD8(d_out, 0, OUTPUT_BYTES), "cuMemsetD8 output");

    void* params[] = { &d_dummy, &d_out };
    check(cuLaunchKernel(fn, 1, 1, 1, N_THREADS, 1, 1, 0, nullptr, params, nullptr),
          "cuLaunchKernel");
    check(cuCtxSynchronize(), "cuCtxSynchronize");
    check(cuMemcpyDtoH(out.data(), d_out, OUTPUT_BYTES), "cuMemcpyDtoH output");

    cuMemFree(d_out);
    cuMemFree(d_dummy);
    cuModuleUnload(module);
    return out;
}

static int report(const char* label, const std::vector<uint32_t>& out) {
    int mismatches = 0;
    for (int tid = 0; tid < N_THREADS; ++tid) {
        uint32_t got = out[tid * OUTPUT_WORDS_PER_THREAD];
        uint32_t expected = static_cast<uint32_t>(tid) | 1u;
        if (got != expected) {
            if (mismatches < 8) {
                std::printf("%s tid %2d: got 0x%08x expected 0x%08x\n",
                            label, tid, got, expected);
            }
            ++mismatches;
        }
    }
    std::printf("%s mismatches: %d / %d\n", label, mismatches, N_THREADS);
    return mismatches;
}

int main(int argc, char** argv) {
    try {
        const char* env_ptxas = std::getenv("PTXAS");
        std::string ptxas = env_ptxas ? env_ptxas : "/usr/local/cuda/bin/ptxas";
        if (access(ptxas.c_str(), X_OK) != 0) {
            ptxas = "ptxas";
        }
        std::string arch = (argc >= 2) ? argv[1] : "sm_103";

        std::cout << "Using ptxas: " << ptxas << "\n";
        std::cout << "Using arch:  " << arch << "\n";
        std::cout << "Expected slot0[tid]: tid | 1\n\n";

        auto cubin_o0 = compile_ptx(ptxas, arch, "-O0");
        auto cubin_o1 = compile_ptx(ptxas, arch, "-O1");
        auto cubin_o2 = compile_ptx(ptxas, arch, "-O2");
        auto cubin_o3 = compile_ptx(ptxas, arch, "-O3");

        check(cuInit(0), "cuInit");
        CUdevice dev = 0;
        CUcontext ctx = nullptr;
        check(cuDeviceGet(&dev, 0), "cuDeviceGet");
        create_context(&ctx, dev);

        int bad_o0 = report("-O0", run_kernel(cubin_o0));
        int bad_o1 = report("-O1", run_kernel(cubin_o1));
        int bad_o2 = report("-O2", run_kernel(cubin_o2));
        int bad_o3 = report("-O3", run_kernel(cubin_o3));

        cuCtxDestroy(ctx);

        if (bad_o0 == 0 && (bad_o1 != 0 || bad_o2 != 0 || bad_o3 != 0)) {
            std::cout << "\nREPRODUCED: -O0 matches the scalar PTX trace, but optimized ptxas is wrong.\n";
            return 1;
        }
        if (bad_o0 != 0) {
            std::cout << "\nUnexpected: -O0 did not match the scalar PTX trace.\n";
            return 2;
        }
        std::cout << "\nNot reproduced: optimized ptxas matched the scalar PTX trace on this setup.\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "error: " << e.what() << "\n";
        return 2;
    }
}
