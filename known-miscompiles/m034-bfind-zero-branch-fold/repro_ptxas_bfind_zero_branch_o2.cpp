...
//
// Run:
//   ./repro_ptxas_bfind_zero_branch_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_bfind_zero_branch_o2 sm_103
//
// Correct PTX behavior for lane tid.x == 0:
//   p0  = setp.ne.u32 tid.x, 0        = false
//   r16 = bfind.u32 0                 = 0xffffffff
//   r4  = xor.b32 r16, tid.x          = 0xffffffff
//   r3  = sub.u32 r4, 32              = 0xffffffdf
//   store r3 to out[0][3]
//
// For tid.x != 0, the branch skips the bfind and stores 4 - 32 = 0xffffffe4.
//
// ptxas -O0 stores 0xffffffdf for lane 0. With affected ptxas versions,
// ptxas -O2 stores 0xffffffe0, as if bfind.u32 0 had folded to 0 instead of
// 0xffffffff. This reproduced on 2026-05-15 with CUDA Toolkit 13.0 ptxas
...
constexpr uint32_t EXPECTED = 0xffffffdfu;
constexpr uint32_t WRONG = 0xffffffe0u;
constexpr size_t OUTPUT_BYTES = 16;
constexpr size_t TARGET_OFFSET = 12;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_bfind_zero_repro.XXXXXX");
