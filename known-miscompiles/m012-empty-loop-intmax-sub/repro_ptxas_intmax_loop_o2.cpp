...
//   g++ -std=c++17 -O2 repro_ptxas_intmax_loop_o2.cpp \
//     -I/usr/local/cuda/include -L/usr/local/cuda/lib64/stubs -lcuda \
//     -o repro_ptxas_intmax_loop_o2
//
...
//
// Run:
//   ./repro_ptxas_intmax_loop_o2 [sm_XX]
//
// Optional:
//   PTXAS=/path/to/ptxas ./repro_ptxas_intmax_loop_o2 sm_103
//
// Correct scalar behavior:
//   The first counted loop executes once and only decrements its counter. The
//   second counted loop executes four times. Each iteration sets:
//
//     r0 = 0x7fffffff
//
//   After the loop, the source computes:
//
//     r3 = 430 - r0 = 430 - 0x7fffffff = 0x800001af  (mod 2^32)
//
//   The correct stored value is therefore 0x800001af.
//
// Observed bug with CUDA 13.0 ptxas V13.0.88 on sm_103 and with CUDA 13.2
// Update 1 ptxas V13.2.78 on sm_103:
//   -O0 stores 0x800001af.
//   -O2 stores 0x800001ad, as if the loop value were 0x80000001 instead of
//   0x7fffffff.
//
// SASS root-cause summary:
//   At -O0, ptxas keeps the loops and emits `IADD3 ..., -R0, 0x1ae, ...` for
//   the final subtraction after the loop writes R0 = 0x7fffffff.
//   At -O2, ptxas deletes both loops and stores the wrong constant
//   0x800001ad directly. Removing the preceding empty counted loop or reducing
//   the value loop from four trips to three makes -O2 correct.
...
constexpr uint32_t EXPECTED = 0x800001afu;
...
static std::vector<char> compile_ptx(const std::string& ptxas,
                                     ...
                                     const char* opt) {
    TempDir dir("/tmp/ptxas_intmax_loop_repro.XXXXXX");
...
static uint32_t run_kernel(const std::vector<char>& cubin) {
    ...
    CUdeviceptr d_unused = 0;
    void* params[] = { &d_unused, &d_out };
