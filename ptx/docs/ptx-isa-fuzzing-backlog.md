...
| Half precision floating point | `add`, `sub`, `mul`, `fma`, `abs`, `neg`, `min`, `max`, `set`, `setp`, `setp.<cmp>.<and|or|xor>`, `selp.b16`, and conversion chains on `.f16`; arithmetic plus `cvt.rn.f16x2.f32` on `.f16x2` | Deterministic finite constants are emitted in the entry prologue and folded into existing u32 outputs; scalar `.f16` arithmetic, packed `.f16x2` arithmetic, scalar compare/select forms, and `.f16`/`.f16x2` conversion chains are also randomized in generated bodies with finite synthesized operands and predicated variants. |
...
| --- | --- | --- | --- | --- |
| High | More half precision dataflow | More packed `.f16x2` predicate/compare and select forms | The deterministic prologue covers basic arithmetic, scalar compare materialization, predicate-fed selection, representative conversion chains, randomized scalar `.f16` and packed `.f16x2` arithmetic, full scalar ordered/unordered compare/setp, setp boolean forms, predicated scalar select, and randomized `.f16`/`.f16x2` conversion chains. Packed compare/select dataflow is still plausible with finite synthesized lane values. | Add packed predicate/select helpers and keep exact-or-approx oracle policy explicit. |
| High | More packed conversions | Narrower integer pack/unpack forms beyond `cvt.pack` and richer randomized placement for existing f16/bf16/tf32 conversions | The deterministic prologue now covers representative saturating `cvt.pack`, f16, bf16, bf16x2, and tf32 conversion forms; other packed conversions are still mostly pure dataflow and optimizer-heavy. | Add typed scratch registers and avoid approximate/boundary cases unless marked intentionally inexact. |
...
1. Add packed half-precision compare/select dataflow, starting with exact finite
   `.f16x2` lane inputs and excluding approximations until the oracle is defined.
