...
| Data movement and conversion | `mov`, `ld`, `ld.global.nc`, `ldu`, `st`, `prefetch`, `prefetchu`, `createpolicy`, `applypriority`, `prmt`, `cvta`, `cvt`, `cvt.pack`, `isspacep` | Covers globals, const, local, shared, generic addresses, vectors, cache operators, prefetch hints, cache-policy creation and cache-hint loads/stores, volatile forms, narrow/wide integer conversion, f32/f64/f16/bf16/tf32 conversion, saturating integer packing, and special registers. |
...
| Device helper calls | `call.uni` and `call` to generated `.func` helpers | Deterministic leaf helpers are emitted before the entry kernel and called from the uniform prologue, including single-return, multi-argument, and multi-return forms whose return values feed existing outputs. |
...
| Medium | More device helper calls | Explicit `.param` ABI call patterns, nested but acyclic helper calls, and richer helper bodies | The prologue now covers single-return, multi-argument, multi-return, `call.uni`, and non-`uni` call spellings in a uniform region. Additional ABI shapes can still stress inliner, ABI lowering, and register passing. | Expand the generated `.func` library and marshal params/returns without recursion. |
...
| Medium | More cache policy consumers | `atom` and `red` forms using `.L2::cache_hint` policies | The prologue now covers `createpolicy`, `applypriority`, regular and non-coherent cache-hint loads, and cache-hint stores; atom/red consumers are deterministic if pointed at existing per-thread memory regions but overlap known atomic/reduction suppressors. | Thread a generated policy register through selected atomic/reduction emitters when those families are enabled. |
