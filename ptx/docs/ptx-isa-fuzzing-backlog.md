...
| Data movement and conversion | `mov`, `ld`, `ld.global.nc`, `ldu`, `st`, `prefetch`, `prefetchu`, `createpolicy`, `applypriority`, `prmt`, `cvta`, `cvt`, `cvt.pack`, `isspacep` | Covers globals, const, local, shared, generic addresses, vectors, cache operators, prefetch hints, cache-policy creation and cache-hint loads/stores/atomics/reductions, volatile forms, narrow/wide integer conversion, deterministic and randomized f32/f64/f16/bf16/tf32 conversion, deterministic and randomized saturating integer packing, and special registers. |
...
| Half precision floating point | `add`, `sub`, `mul`, `fma`, `abs`, `neg`, `min`, `max`, `set`, `setp`, `setp.<cmp>.<and|or|xor>`, `selp.b16`, and conversion chains on `.f16`; arithmetic, `set`, `setp`, `setp.<cmp>.<and|or|xor>`, predicate-fed `selp.b32`, and `cvt.rn.f16x2.f32` on `.f16x2` | Deterministic finite constants are emitted in the entry prologue and folded into existing u32 outputs; scalar `.f16` arithmetic, packed `.f16x2` arithmetic, scalar and packed compare/select forms, and `.f16`/`.f16x2` conversion chains are also randomized in generated bodies with finite synthesized operands and predicated variants. |
...
| Warp collectives | `activemask`, `vote.sync`, `match.sync`, `elect.sync`, `shfl.sync`, `redux.sync` | Full-mask forms are emitted in the deterministic entry prologue and randomized uniform-prefix islands before generated divergent control flow, so all warp lanes participate and results are deterministic. |
| Device helper calls | `call.uni` and `call` to generated `.func` helpers | Deterministic helpers are emitted before the entry kernel and called from the uniform prologue, including register ABI, explicit `.param` ABI, mixed register/`.param` signatures, 32-bit, 64-bit, f32/f64, and mixed-width returns, predicate/select helper bodies, single-return, multi-argument, multi-return, and nested acyclic forms whose return values feed existing outputs. Randomized uniform-prefix helper-call islands re-invoke the same variants with both `call.uni` and `call` spellings so the inliner, ABI lowering, and register passing see varied insertion points. |
| Control flow | `bra`, `brx.idx`, predicated instructions, structured braces, `ret` | Generator emits arbitrary CFG or structured if/loop shapes with bounded loop counters, plus bounded prologue and in-body branch tables whose targets rejoin normally. |
| Special registers and predefined constants | `%tid`, `%ntid`, `%ctaid`, `%nctaid`, `%laneid`, `%nwarpid`, `WARP_SZ`, `%lanemask_*`, `%clusterid`, `%nclusterid`, `%cluster_ctaid`, `%cluster_nctaid`, `%cluster_ctarank`, `%cluster_nctarank`, `%is_explicit_cluster` | Read as deterministic scalar inputs; predicated forms exist for some paths. Cluster registers use the default single-cluster launch shape and are compared only between optimization levels for the same launch. |

## Candidates

| Priority | ISA area | Candidate instructions | Why it is fuzzable | Main implementation work |
| --- | --- | --- | --- | --- |
| High | More packed conversions | Narrower integer pack/unpack forms beyond `cvt.pack` and additional packed conversion type combinations | The deterministic prologue and randomized bodies now cover representative saturating `cvt.pack`, f16, bf16, bf16x2, and tf32 conversion forms. Other packed conversions are still mostly pure dataflow and optimizer-heavy. | Add typed scratch registers and avoid approximate/boundary cases unless marked intentionally inexact. |
...
| High | More warp collective dataflow | Additional in-body uniform-island placements for the existing warp collectives | Full-mask deterministic and randomized uniform-prefix forms are covered; further coverage is plausible if the emitter can guarantee uniform participation after local rejoin points. | Add a reusable in-body uniform-region emitter. |
| Medium | More branch table control flow | Denser or nested `brx.idx` tables | Prologue and single in-body branch tables are covered; denser tables are safe if the index is masked to an in-range table and every target rejoins normally. | Add higher-density table placement and variants after clean fuzz intervals. |
| Medium | More device helper calls | Richer helper bodies and additional type signatures | The prologue now covers single-return, multi-argument, multi-return, explicit `.param` ABI, mixed register/`.param` signatures, 64-bit, f32/f64, and mixed-width register returns, predicate/select helper bodies, nested acyclic calls, `call.uni`, and non-`uni` call spellings in a uniform region; randomized helper-call islands re-invoke the same variants from a randomized point in the uniform prefix. Additional ABI shapes can still stress inliner, ABI lowering, and register passing. | Expand the generated `.func` library and marshal params/returns without recursion. |
...
| Medium | New packed integer types | `.u8x4` / `.s8x4` `add`, `sub`, `min`, `max`, `neg` on targets that support them | PTX 9.2 calls these out as new instruction types, but ptxas 13.2.78 rejects them for the current `sm_103` target. They become deterministic byte-lane candidates once the harness target supports them. | Add target-gated byte-lane packing helpers and suppressor flags distinct from existing video/packed16 paths. |
...
| Destructive cache discard | `discard.global.L2` | It writes unstable indeterminate values to the addressed cache line, so using it on the current input/output buffers can make later exact-output comparisons undefined. |

## Suggested next slices

1. Extend warp-uniform islands beyond the entry prefix by proving local rejoin
   points and inserting full-mask collectives only where all lanes participate.
2. Add denser or nested `brx.idx` insertion points after the next long clean fuzz interval.
