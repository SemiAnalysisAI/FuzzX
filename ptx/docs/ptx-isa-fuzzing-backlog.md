...
| Half precision floating point | `add`, `sub`, `mul`, `fma`, `abs`, `neg`, `min`, `max`, `set`, `setp`, `selp.b16`, and conversion chains on `.f16`; arithmetic plus `cvt.rn.f16x2.f32` on `.f16x2` | Deterministic finite constants are emitted in the entry prologue and folded into existing u32 outputs; scalar `.f16` arithmetic is also randomized in generated bodies with finite synthesized operands and predicated variants. |
...
| --- | --- | --- | --- | --- |
| High | More half precision dataflow | In-body `.f16x2` arithmetic, more predicate/compare variants, and randomized f16 conversion chains | The deterministic prologue covers basic arithmetic, scalar compare materialization, predicate-fed selection, representative conversion chains, and randomized scalar `.f16` arithmetic; richer packed and conversion placement is still plausible with sanitized finite values. | Add packed half value synthesis, pack/unpack helpers, and exact-or-approx oracle policy. |
