...
| Device helper calls | `call.uni` and `call` to generated `.func` helpers | Deterministic helpers are emitted before the entry kernel and called from the uniform prologue, including register ABI, explicit `.param` ABI, mixed register/`.param` signatures, 32-bit and 64-bit returns, predicate/select helper bodies, single-return, multi-argument, multi-return, and nested acyclic forms whose return values feed existing outputs. |
...
| Medium | More device helper calls | Richer helper bodies and additional type signatures | The prologue now covers single-return, multi-argument, multi-return, explicit `.param` ABI, mixed register/`.param` signatures, 64-bit register returns, predicate/select helper bodies, nested acyclic calls, `call.uni`, and non-`uni` call spellings in a uniform region. Additional ABI shapes can still stress inliner, ABI lowering, and register passing. | Expand the generated `.func` library and marshal params/returns without recursion. |
