...
| Data movement and conversion | `mov`, `ld`, `ld.global.nc`, `ldu`, `st`, `prefetch`, `prefetchu`, `createpolicy`, `applypriority`, `prmt`, `cvta`, `cvt`, `cvt.pack`, `isspacep` | Covers globals, const, local, shared, generic addresses, vectors, cache operators, prefetch hints, cache-policy creation and cache-hint loads, volatile forms, narrow/wide integer conversion, f32/f64/f16/bf16/tf32 conversion, saturating integer packing, and special registers. |
...
| Medium | More cache policy consumers | Additional `ld.global.nc`, `st`, `atom`, and `red` forms using `.L2::cache_hint` policies | The prologue now covers `createpolicy`, `applypriority`, and a cache-hint load; more consumers are deterministic if pointed at the existing bounded per-thread memory regions. | Thread a generated policy register through selected memory emitters without changing memory ordering or output determinism. |
