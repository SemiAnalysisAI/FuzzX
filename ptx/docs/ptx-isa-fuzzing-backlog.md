...
| Half precision floating point | `add`, `sub`, `mul`, `fma`, `abs`, `neg`, `min`, `max`, `set`, `setp`, `selp.b16` on `.f16`; arithmetic on `.f16x2` | Deterministic finite constants are emitted in the entry prologue and folded into existing u32 outputs. |
