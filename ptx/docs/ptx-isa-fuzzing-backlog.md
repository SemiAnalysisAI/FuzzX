...
| --- | --- | --- | --- | --- |
| High | More half precision dataflow | Randomized f16 conversion chains and more packed predicate/compare forms | The deterministic prologue covers basic arithmetic, scalar compare materialization, predicate-fed selection, representative conversion chains, randomized scalar `.f16` and packed `.f16x2` arithmetic, full scalar ordered/unordered compare/setp, setp boolean forms, and predicated scalar select. Richer conversion placement is still plausible with sanitized finite values. | Add conversion placement helpers and keep exact-or-approx oracle policy explicit. |
