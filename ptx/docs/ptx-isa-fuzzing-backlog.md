...
| Uniform synchronization | `bar.warp.sync`, `bar.sync`, `barrier.sync` | Emitted in the entry prologue with full-warp or full-CTA participation before generated divergent control flow, so all 32 lanes reach the same barrier. |
...
| Medium | Uniform synchronization | Barrier arrive/reduction variants in uniform regions | Barriers are safe only when all participating threads reach them. | Add more uniform-only insertion points and avoid divergent/early-exit paths. |
