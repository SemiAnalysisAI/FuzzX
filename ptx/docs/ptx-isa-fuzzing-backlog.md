...
| --- | --- | --- | --- | --- |
| High | More half precision dataflow | In-body `.f16x2` arithmetic and randomized f16 conversion chains | The deterministic prologue covers basic arithmetic, scalar compare materialization, predicate-fed selection, representative conversion chains, randomized scalar `.f16` arithmetic, full scalar ordered/unordered compare/setp, setp boolean forms, and predicated scalar select. Richer packed and conversion placement is still plausible with sanitized finite values. | Add packed half value synthesis, pack/unpack helpers, and exact-or-approx oracle policy. |
