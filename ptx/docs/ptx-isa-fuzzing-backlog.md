...
| Uniform synchronization | `bar.warp.sync` | Emitted once in the entry prologue with a full-warp mask before generated divergent control flow, so all 32 lanes reach the same barrier. |
...
| Medium | Uniform synchronization | `bar.sync` and additional barrier variants in uniform regions | CTA barriers are safe only when all participating threads reach them. | Add more uniform-only insertion points and avoid divergent/early-exit paths. |
