...
| Device helper calls | `call.uni` and `call` to generated `.func` helpers | Deterministic helpers are emitted before the entry kernel and called from the uniform prologue, including register ABI, explicit `.param` ABI, mixed register/`.param` signatures, 32-bit, 64-bit, f32/f64, and mixed-width returns, predicate/select helper bodies, single-return, multi-argument, multi-return, and nested acyclic forms whose return values feed existing outputs. |
...
| --- | --- | --- | --- | --- |
| High | More packed conversions | Narrower integer pack/unpack forms beyond `cvt.pack` and additional packed conversion type combinations | The deterministic prologue now covers representative saturating `cvt.pack`, f16, bf16, bf16x2, and tf32 conversion forms; f16/bf16/tf32 conversions also have randomized in-body placement. Other packed conversions are still mostly pure dataflow and optimizer-heavy. | Add typed scratch registers and avoid approximate/boundary cases unless marked intentionally inexact. |
