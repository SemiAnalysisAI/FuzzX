...
| Device helper calls | `call.uni` and `call` to generated `.func` helpers | Deterministic leaf helpers are emitted before the entry kernel and called from the uniform prologue, including register ABI, explicit `.param` ABI, single-return, multi-argument, and multi-return forms whose return values feed existing outputs. |
...
| Medium | More device helper calls | Nested but acyclic helper calls, mixed register/`.param` signatures, and richer helper bodies | The prologue now covers single-return, multi-argument, multi-return, explicit `.param` ABI, `call.uni`, and non-`uni` call spellings in a uniform region. Additional ABI shapes can still stress inliner, ABI lowering, and register passing. | Expand the generated `.func` library and marshal params/returns without recursion. |
