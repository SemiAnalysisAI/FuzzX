...
impl F16ArithOp {
    fn mnemonic(self) -> &'static str {
        match self {
            F16ArithOp::Add => "add.rn.f16",
            F16ArithOp::Sub => "sub.rn.f16",
            F16ArithOp::Mul => "mul.rn.f16",
            F16ArithOp::Fma => "fma.rn.f16",
            F16ArithOp::Min => "min.f16",
            F16ArithOp::Max => "max.f16",
            F16ArithOp::Abs => "abs.f16",
            F16ArithOp::Neg => "neg.f16",
        }
    }
...
enum Inst {
    ...
    /// Randomized scalar half-precision arithmetic through finite `.b16` scratch values.
    F16Arith {
        op: F16ArithOp,
        dst: u32,
        a: Operand,
...
impl<'a> Generator<'a> {
    ...
    fn pick_f16_arith(&mut self, u: &mut Unstructured) -> Result<Inst> {
        let ops = [
            F16ArithOp::Add,
            F16ArithOp::Sub,
            F16ArithOp::Mul,
    ...
    fn emit_prologue(&self, s: &mut String) {
        ...
        if self.cfg.emit_rich_helper_calls {
            ...
            .unwrap();
            writeln!(s, "{{").unwrap();
            writeln!(s, "    .reg .b32       %nhr<4>;").unwrap();
            writeln!(
                s,
            ...
            .unwrap();
            writeln!(s, "    xor.b32         %nhr3, %nhr2, a;").unwrap();
            writeln!(s, "    add.u32         ret0, %nhr3, b;").unwrap();
            writeln!(s, "    ret;").unwrap();
            writeln!(s, "}}").unwrap();
            writeln!(s).unwrap();
            writeln!(
            ...
            .unwrap();
            writeln!(s, "{{").unwrap();
            writeln!(s, "    .reg .b64       %mrr<2>;").unwrap();
            writeln!(s, "    add.u32         ret0, a, c;").unwrap();
            writeln!(s, "    cvt.u64.u32     %mrr0, ret0;").unwrap();
            writeln!(s, "    shl.b64         %mrr1, %mrr0, 7;").unwrap();
            writeln!(s, "    xor.b64         ret1, b, %mrr1;").unwrap();
            writeln!(s, "    ret;").unwrap();
            writeln!(s, "}}").unwrap();
            writeln!(s).unwrap();
            writeln!(
            ...
            .unwrap();
            writeln!(s, "{{").unwrap();
            writeln!(s, "    mov.b64         ret0, a;").unwrap();
            writeln!(s, "    ret;").unwrap();
            writeln!(s, "}}").unwrap();
            writeln!(s).unwrap();
            writeln!(
        ...
        if self.cfg.emit_rich_helper_calls {
            ...
            writeln!(s, "    add.u32         %r0, %r0, %r{scratch};").unwrap();
            writeln!(s, "    mov.b64         %fd0, 0d3ff0000000000000;").unwrap();
            writeln!(s, "    mov.b64         %fd1, 0d4000000000000000;").unwrap();
            writeln!(
                s,
        ...
        if self.cfg.emit_f16_arith || self.cfg.emit_f16_compare || self.cfg.emit_f16_cvt {
        ...
        if self.cfg.emit_f16_cvt {
            let scratch = self.wide_scratch_hi_reg();
            writeln!(s, "    cvt.f32.f16     %f0, %h0;").unwrap();
            writeln!(s, "    cvt.f32.f16     %f1, %h1;").unwrap();
            writeln!(s, "    cvt.rn.f16.f32  %h2, %f0;").unwrap();
            writeln!(s, "    cvt.f64.f16     %fd0, %h2;").unwrap();
            writeln!(s, "    cvt.rn.f16.f64  %h3, %fd0;").unwrap();
            writeln!(s, "    cvt.rzi.u32.f16 %r{scratch}, %h3;").unwrap();
            writeln!(s, "    add.u32         %r0, %r0, %r{scratch};").unwrap();
            writeln!(s, "    cvt.rzi.s32.f16 %r{scratch}, %h3;").unwrap();
            writeln!(s, "    add.u32         %r0, %r0, %r{scratch};").unwrap();
            writeln!(s, "    mov.u32         %r{scratch}, 3;").unwrap();
            writeln!(s, "    cvt.rn.f16.u32  %h2, %r{scratch};").unwrap();
            writeln!(s, "    cvt.rn.f16.s32  %h3, %r{scratch};").unwrap();
            writeln!(s, "    cvt.u32.u16     %r{scratch}, %h2;").unwrap();
            writeln!(s, "    add.u32         %r0, %r0, %r{scratch};").unwrap();
            writeln!(s, "    cvt.u32.u16     %r{scratch}, %h3;").unwrap();
            writeln!(s, "    add.u32         %r0, %r0, %r{scratch};").unwrap();
    ...
    fn emit_sanitized_f16_operand(&self, s: &mut String, hreg: u32, operand: Operand) {
        let scratch = self.wide_scratch_hi_reg();
        write!(s, "    and.b32       %r{scratch}, ").unwrap();
        operand.emit(s);
    ...
    fn emit_inst(&self, s: &mut String, inst: &Inst) {
        match *inst {
            ...
            Inst::F16Arith { op, dst, a, b, c } => {
                self.emit_sanitized_f16_operand(s, 0, a);
                if !op.is_unary() {
                    self.emit_sanitized_f16_operand(s, 1, b);
                }
                if op.uses_c() {
                    self.emit_sanitized_f16_operand(s, 2, c);
                    writeln!(s, "    {:<13} %h3, %h0, %h1, %h2;", op.mnemonic()).unwrap();
                } else if op.is_unary() {
                    writeln!(s, "    {:<13} %h3, %h0;", op.mnemonic()).unwrap();
                } else {
            ...
            Inst::PredicatedF16Arith {
                ...
                writeln!(s, "    mov.b16       %h3, %h0;").unwrap();
                if op.uses_c() {
                    self.emit_sanitized_f16_operand(s, 2, c);
                    writeln!(
                        s,
                    ...
                    writeln!(s, "    {} {:<8} %h3, %h0;", pred_guard(pred), op.mnemonic()).unwrap();
                } else {
                    writeln!(
                        s,
                        "    {} {:<8} %h3, %h0, %h1;",
...
mod tests {
    ...
    const F16_SCALAR_ARITH_MNEMONICS: &[&str] = &[
        "add.rn.f16",
        "sub.rn.f16",
        "mul.rn.f16",
        "fma.rn.f16",
    ...
    #[test]
    fn cta_barrier_generation_is_reachable() {
        let ptx = generate_from_bytes(&bytes_from_seed(0, 4096)).unwrap();
    ...
    #[test]
    fn cta_barrier_generation_can_be_disabled() {
        let cfg = GenConfig {
            emit_cta_barriers: false,
            ..coverage_heavy_config()
        };
    ...
    fn rich_helper_call_generation_is_reachable() {
        ...
        assert!(
            ptx.contains("fuzzx_nested_helper"),
            "missing nested helper function"
        );
        assert!(
        ...
        assert!(
            ...
            "missing mixed-return helper function"
        );
        assert!(
            ptx.contains("call.uni        (%r1, %rd8), fuzzx_mixed_return_helper"),
        ...
        assert!(
            ptx.contains("fuzzx_f64_bit_helper"),
            "missing f64 bit-preserving helper function"
        );
        assert!(
            ptx.contains("call.uni        (%fd2), fuzzx_f64_bit_helper"),
            "missing f64 bit-preserving helper call"
        );
        assert!(
    ...
    fn rich_helper_call_generation_can_be_disabled() {
        ...
        for seed in 0..128 {
            ...
            assert!(
                !ptx.contains("fuzzx_nested_helper"),
                "seed {seed:x} emitted nested helper function"
            );
            assert!(
                !ptx.contains("call            (%nhr2), fuzzx_helper_chain"),
            ...
            assert!(
                !ptx.contains("fuzzx_f64_bit_helper"),
                "seed {seed:x} emitted f64 bit-preserving helper function"
            );
            assert!(
                !ptx.contains("fuzzx_param_helper"),
    ...
    #[test]
    fn randomized_f16_arith_generation_is_reachable() {
        let cfg = GenConfig {
            emit_scalar_16bit: false,
        ...
        let mut found_sanitized_f16_input = false;
        for seed in 0..8192 {
            let bytes = bytes_from_seed(seed, 8192);
            let ptx = generate_from_bytes_with_config(&bytes, &cfg).unwrap();
            if ptx.contains("cvt.u16.u32   %h0") {
                found_sanitized_f16_input = true;
                break;
            }
        }
        assert!(
            found_sanitized_f16_input,
            "sample did not emit randomized f16 arithmetic input synthesis"
        );
    }

    #[test]
    fn predicated_randomized_f16_arith_generation_is_reachable() {
        let cfg = GenConfig {
            emit_scalar_16bit: false,
            emit_f16_compare: false,
            emit_f16_cvt: false,
    ...
    #[test]
    fn f16_cvt_generation_can_be_disabled() {
        let cfg = GenConfig {
            emit_f16_cvt: false,
            ..coverage_heavy_config()
        };
        ...
        let ptx = generate_from_bytes_with_config(&bytes_from_seed(0, 4096), &cfg).unwrap();
        for mnemonic in F16_CVT_MNEMONICS {
            assert!(
                !has_mnemonic(&ptx, mnemonic),
                "disabled f16 conversion still emitted {mnemonic}"
            );
        }
    }
