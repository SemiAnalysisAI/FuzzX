...
//!
//! Usage:
//!   fuzzx-diff-sweep <base.ptx> <input.bin> [candidates.txt]
//!
...
use fuzzx_execgen::{input_len, output_len, KERNEL_NAME, N_THREADS, TARGET_ARCH};
...
fn main() -> Result<()> {
    ...
    let base_path = PathBuf::from(std::env::args().nth(1).ok_or_else(|| {
        anyhow!("usage: fuzzx-diff-sweep <base.ptx> <input.bin> [candidates.txt]")
    })?);
    let input_path = PathBuf::from(std::env::args().nth(2).ok_or_else(|| {
        anyhow!("usage: fuzzx-diff-sweep <base.ptx> <input.bin> [candidates.txt]")
    })?);
    ...
    eprintln!(
        "fuzzx-diff-sweep: base={} candidates={} gpus={:?} workers_per_gpu={} (total={})",
        base_path.display(),
        candidates.len(),
        gpus,
        workers_per_gpu,
    ...
    let assigned_at_stop = loop {
        ...
        if in_f > 0 && last_progress_at.elapsed() >= no_progress_timeout {
            eprintln!(
                "fuzzx-diff-sweep: no candidate finished for {:?}; abandoning {} in-flight workers",
        ...
        if assigned >= n {
            ...
            if started.elapsed() >= grace {
                eprintln!(
                    "fuzzx-diff-sweep: giving up on {} in-flight workers after {:?} grace",
                    in_f, grace,
                );
                break assigned;
            }
