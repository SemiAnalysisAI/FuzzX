// CUDA inline-PTX variant of the m053-bfi-reg-len-fold ptxas reproducer.
//
...
constexpr int kInputWords = 1;
constexpr int kOutputWords = 4;
constexpr uint32_t kN = 32u;
constexpr uint32_t kInput0 = 0xc58c427cu;
...
int main() {
    uint32_t h_in[kInputWords] = {kInput0};
    ...
    std::printf("threads=%d n=%u input0=0x%08x\n", kThreads, kN, kInput0);
    for (int i = 0; i < kOutputWords; ++i) {
